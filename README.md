...
Playwright 会自动启动 Vite（端口 4173），串行验证 Dashboard、编辑器审批配置、双区域校验、条件修复、动态表单预览、发布联动、异常实例详情、版本比较和历史恢复、模板派生（编号重排 / 溯源映射 / 来源筛选 / 归档不破坏映射 / 副本发布校验）。测试失败时会保留截图和 trace。
...
- `src/lib/derivation.ts`：模板派生纯规则（编号重排、来源与节点/连线映射、副本 v0 起版、派生统计），与数据和页面解耦。
- `src/store/`：Zustand 全局状态、校验、发布、派生、归档及恢复逻辑。
- `src/types/`：核心业务数据 TypeScript 类型（含 `DerivedFrom` 溯源结构）。
- `tests/`：关键产品链路 Playwright 测试。

## 模板派生台

流程列表的派生按钮（GitFork 图标）会把当前流程作为模板派生出一份可追溯副本：

- 副本、节点（`node-…`）和连线（`edge-…`）全部重新编号，端点按映射重写，配置深拷贝，与源彻底解耦；
- 副本从草稿 v0 开始，不携带源版本历史；发布仍走现有校验，通过后生成副本自己的版本；
- `derivedFrom` 记录源流程 id/名称、派生时的原始版本号、派生时间以及节点、连线编号映射；
- 同一模板可派生任意多份，后续配置互不影响；列表“派生”列显示次数（点击可按来源筛选），工具栏支持仅看派生副本/原始模板/指定来源；
- 版本页顶部展示派生来源、原始版本与完整编号映射（源归档后映射仍可追溯），原始模板的版本页则列出它的全部副本；
- 归档源流程不修改副本任何数据；副本发布、校验、恢复均沿用既有链路。
